...
parser.add_argument('Datatype', metavar='datatype', type=str, action='store')
...
class ReadAlignment:
    ...
    def read_file(self, filepath):
        ...
        if amount_of_seq > 500000:
            sys.exit("This might take a while")
        len1 = len(sequences[0])
        if len1 <= 1:
            sys.exit("Check fasta file, it may be empty")
        if len1 > 1000000:
            print('This might take some time')
            print('restart with shorter sequence')
        counter = 0
        ...
        if (counter == amount_of_seq):
            print("All the sequences are the same length.")
            return (name, sequences, amount_of_seq)
        else:
            sys.exit("Sequence lengths not matched")
...
class TransitionTable:
    ...
    def bin_table(self):
        ...
        #This turns my data into a dataframe consisting of 0's and 1's to represent dashes and letters

# So now I need to determine conserved regions
    ...
    def querysequence(self, insert_reg):
        ...
        if sample_type == 'P':
            ...
            if 'J' in query or 'O' in query or 'U' in query or 'X' in query:
                sys.exit("Contains invalid character?")  # elif check whether any special char except '-' is present OR any numbers
            else:
            ...
            sys.exit("Retype the query sequence")
        s = 0
        i = 0
        region = 0
        bi_o = []
